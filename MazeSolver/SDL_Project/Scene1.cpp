...
bool Scene1::OnCreate() {
	...
	//SDL_Surface* birdImage = IMG_Load("ball.png");
...
void Scene1::Update(const float deltaTime) {
	...
	printf("%f %f %f\n", bird->getAccel().y, bird->getVel().y, bird->getPos().y);
	angle = atan2(bird->getVel().y, bird->getVel().x);
...
void Scene1::Render() {
	...
	//Vec3 screenCoords;
	//int w, h;
		//SDL_QueryTexture(bird->getTexture(), nullptr, nullptr, &w, &h);
		//screenCoords = projectionMatrix * bird->getPos();
		//square.x = static_cast<int>(screenCoords.x);
		//square.y = static_cast<int>(screenCoords.y);
		//square.w = bird->getImage()->w;
		//square.h = bird->getImage()->h;
		//SDL_RenderCopyEx(renderer, bird->getTexture(), nullptr, &square,
